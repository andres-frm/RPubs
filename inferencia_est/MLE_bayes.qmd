...
:::{}
i. Iteramos sobre cada $observación_i$ (`obs[i]`) para generar una distribución posterior ($P(\phi | datos)$) dado el producto de la verosimilitud ($P(datos | \phi)$ = `dbinom(obs[i], 1, prob = grilla)`) y la distribución previa ($P(\phi)$ = $beta(1, 1)$ = `previa`). En este punto es importante aclarar que:
