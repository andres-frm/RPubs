...
est_p <- sapply(1:100, simplify = 'array', FUN = 
                  function(i) {
                    ...
                    p <- 
                      post$mu + 
                      post[, grepl('alpha', colnames(post))][[i_alpha]] * post$sigma +
                      post[, grepl('psi', colnames(post))][[i_psi]] + post$tau +
...
```{r, warning=FALSE}
par(mfrow = c(1, 2), main = c(4, 4, 1, 1))
...
ppcheck <- sapply(1:length(dat$anticonc), simplify = 'array', FUN = 
                    function(x) {
                      ...
                      p <- 
                        post2[, grepl('alpha', colnames(post2))][[urbano]] + 
                        post2[, grepl('theta', colnames(post2))][[distrito]] + 
                        post2$beta*edad
...
post_urb_dist <- sapply(1:nrow(bar_urb), simplify = 'array', FUN = 
                          function(x) {
                            post2[, grepl('alpha', colnames(post2))][[bar_urb$urb[x]]] +
                              post2[, grepl('theta', colnames(post2))][[bar_urb$dist[x]]]
                          })
...
alpha_urb <- as_tibble(post2[, grepl('alpha', colnames(post2))])
